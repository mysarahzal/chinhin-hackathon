...
// --- INTERFACES (Wajib ada untuk elakkan error di frontend) ---
export interface PricingInput {
  sku: string;
  productName: string;
  customerName: string;
...
const connectionString = import.meta.env.VITE_PROJECT_CONNECTION_STRING;
...
export const getAiRecommendation = async (input: PricingInput): Promise<PricingResult> => {
  
  // 1. Guna .threads.create()
  const thread = await agentsClient.threads.create();
  
  const promptContent = `Recommend price for SKU: ${input.sku}, Tier: ${input.customerTier}, Channel: ${input.salesChannel}. 
              Inventory Aging: ${input.inventoryAging} days. Cost: ${input.costPrice}.
              Sila berikan jawapan dalam format JSON yang mengandungi rationale berdasarkan pricing_rules.md.`;
  ...
  const run = await agentsClient.runs.createAndPoll(thread.id, agentId, {
    ...
    for await (const msg of messages) {
      if (msg.role === "assistant") {
        ...
        if (contentItem && "text" in contentItem) {
          try {
            return JSON.parse(contentItem.text.value) as PricingResult;
          } catch (error) {
...
// Dummy export untuk elakkan error sementara di PricingInput.tsx
export function calculatePricing(input: PricingInput): PricingResult {
  console.warn("Using fallback calculatePricing.");
